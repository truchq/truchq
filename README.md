...
| DApp Browser | Injected `window.ethereum` — no MetaMask needed |
| NFT (ERC-721 / ERC-1155) | Metadata resolution, IPFS gateway, send/receive |
| Offline Transaction Queue | MMKV-persisted, broadcast on reconnect |
| Per-wallet Security | AES-GCM + PBKDF2, Keychain biometric, PIN lockout |
| Portfolio Aggregation | Parallel RPC queries, 60% faster load time |
...
`React Native CLI` · `TypeScript` · `viem` · `Zustand` · `MMKV` · `TanStack Query v5`
`@reown/walletkit` · `react-native-keychain` · `NativeWind v4` · `React Navigation v7`
`@scure/bip32` · `@noble/hashes` · `react-hook-form` · `Zod v4`
